...
def prompts_from_abby(url,drive=gdrive):
    ...
    filtered_variables = [var for var in text if not re.search(pattern, var) and var not in ('Load More Articles' , 'SubscribeReceive Dear Abby Free Every DayEmail address:Subscribe Now')]
...
for k in ['2012','2013','2014']:
    for i in range (1,13):
        for j in range(1, 31):
            ...
            print([k,i,j])
            try:
                final_info.append((prompts_from_abby('https://www.uexpress.com/life/dearabby/{}/{}/{}'.format((str(k)),(str(i)),str(j)))))
            except:
                pass
...
df.to_csv(os.path.join(os.getcwd(), 'results_txt','DearAbbyFinal'+str(datetime.datetime.now().strftime("%Y_%m_%d-%I_%M_%S_%p"))+'.csv'), encoding='utf-8', sep=',')
